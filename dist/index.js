import { Injectable, Optional, Inject, Global, Module, inject } from '@stackra/ts-container';
import { COORDINATOR_CONFIG, TAB_COORDINATOR, TAB_LOCK_MANAGER } from '@stackra/contracts';
...
var LocalStorageFallback = class _LocalStorageFallback {
  ...
  name;
  senderId;
  logger = new Logger(_LocalStorageFallback.name);
  storageKey;
  storageHandler;
  closed = false;
...
var TabCoordinator = class _TabCoordinator {
  static {
    __name(this, "TabCoordinator");
  }
  logger = new Logger(_TabCoordinator.name);
  ...
  */
  ...
  constructor(config = {}) {
    ...
    this.roleSubject = new BehaviorSubject("follower");
...
TabCoordinator = _ts_decorate([
  Injectable(),
  _ts_param(0, Optional()),
  _ts_param(0, Inject(COORDINATOR_CONFIG)),
...
LockManager = _ts_decorate2([
  Injectable(),
  _ts_param2(0, Optional()),
  _ts_param2(0, Inject(COORDINATOR_CONFIG)),
...
var CoordinatorTransport = class _CoordinatorTransport {
  static {
    __name(this, "CoordinatorTransport");
  }
  logger = new Logger(_CoordinatorTransport.name);
...
CoordinatorTransport = _ts_decorate3([
  EventTransport({
    name: "coordinator"
  }),
  Injectable(),
  _ts_param3(0, Optional()),
  _ts_param3(0, Inject(COORDINATOR_CONFIG)),
...
var CoordinatorModule = class _CoordinatorModule {
  static {
  ...
  static forRoot(config = {}) {
    return {
      ...
      providers: [
        ...
        // Core services
        {
          provide: TabCoordinator,
          useClass: TabCoordinator
        },
        {
          provide: TAB_COORDINATOR,
          useExisting: TabCoordinator
        },
        // Lock manager
        {
          provide: LockManager,
          useClass: LockManager
        },
        {
          provide: TAB_LOCK_MANAGER,
          useExisting: LockManager
        },
...
CoordinatorModule = _ts_decorate4([
  Global(),
  Module({})
], CoordinatorModule);
var InjectCoordinator = /* @__PURE__ */ __name(() => Inject(TAB_COORDINATOR), "InjectCoordinator");
var InjectLockManager = /* @__PURE__ */ __name(() => Inject(TAB_LOCK_MANAGER), "InjectLockManager");
...
export { CoordinatorError, CoordinatorModule, CoordinatorTransport, InjectCoordinator, InjectLockManager, LockManager, TabCoordinator, coordinator, defineConfig, lock, useIsLeader, useTabCount };
